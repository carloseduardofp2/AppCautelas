...
export function useMateriais() {
    ...
    // ==========================================
    // --- MODO DE SELEÇÃO MÚLTIPLA E MOVER ---
    // ==========================================
    ...
    function ativarModoSelecao(id) {
        setModoSelecao(true);
        setItensSelecionados([id]);
    }

    function toggleSelecao(id) {
        if (itensSelecionados.includes(id)) {
            const novaLista = itensSelecionados.filter(item => item !== id);
            setItensSelecionados(novaLista);
            if (novaLista.length === 0) setModoSelecao(false); // Sai do modo se desmarcar tudo
        } else {
    ...
    async function confirmarMovimentacao() {
        try {
            const promessas = itensSelecionados.map(id => {
                const ref = doc(db, 'materiais', id);

                // Transforma o array do caminho de volta para as strings antigas que sua tela usa
                const novaLocalizacao = caminhoDestinoMover.length > 0 ? caminhoDestinoMover[0] : 'Não informado';
                const novaSubLocalizacao = caminhoDestinoMover.length > 1 ? caminhoDestinoMover[1] : '';

                return updateDoc(ref, {
                    localizacao: novaLocalizacao,
                    subLocalizacao: novaSubLocalizacao,
                    path: caminhoDestinoMover, // Salva o novo formato junto para o futuro
                    isFolder: false
                });
            });
            ...
            Alert.alert("Sucesso", `${itensSelecionados.length} itens movidos com sucesso!`);

            // Limpa tudo após mover
            ...
            console.error(error);
            Alert.alert("Erro", "Não foi possível mover os itens.");
        }
    }

    // Gera uma lista com base nos locais reais que já existem no seu banco antigo
    const locaisUnicos = [...new Set(listaMateriais.map(m => m.localizacao).filter(l => l && l !== 'Não informado'))];
    const pastasMapeadas = [];

    locaisUnicos.forEach((loc, index) => {
        // Mapeia a Prateleira Principal (Nível 1)
        pastasMapeadas.push({
            nomeExibicao: `📁 ${loc}`,
            pathFuturo: [loc],
            id: `loc-${index}`
        ...
        // Mapeia as sub-prateleiras (Nível 2)
        const subLocais = [...new Set(listaMateriais.filter(m => m.localizacao === loc && m.subLocalizacao).map(m => m.subLocalizacao))];
        subLocais.forEach((sub, subIndex) => {
            pastasMapeadas.push({
                nomeExibicao: `📁 ${loc} › ${sub}`,
                pathFuturo: [loc, sub],
    ...
    const todasAsPastas = [
        { nomeExibicao: '🏠 Raiz Principal (Início)', pathFuturo: [], id: 'raiz' },
        ...pastasMapeadas
    ];
